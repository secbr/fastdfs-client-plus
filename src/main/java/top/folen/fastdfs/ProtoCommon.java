...
public class ProtoCommon {
	...
	public static final byte STORAGE_PROTO_CMD_REGENERATE_APPENDER_FILENAME = 38;  //rename appender file to normal
	// file
	...
	public static byte[] packHeader(byte cmd, long pkg_len, byte errno) {
		...
		hexLen = ProtoCommon.long2buff(pkg_len);
	 ...
	 *
	 * @param metaBuff        metadata
	 * @param recordSeparator record/row seperator
	 * @param filedSeparator  field/column seperator
	 * @return name value pair array
	 */
	public static NameValuePair[] splitMetadata(String metaBuff,
	                                            String recordSeparator, String filedSeparator) {
		String[] rows;
		String[] cols;
		NameValuePair[] metaList;

		rows = metaBuff.split(recordSeparator);
		metaList = new NameValuePair[rows.length];
		for (int i = 0; i < rows.length; i++) {
			cols = rows[i].split(filedSeparator, 2);
	...
	public static String packMetadata(NameValuePair[] metaList) {
		...
		sb.append(metaList[0].getName()).append(FDFS_FIELD_SEPERATOR).append(metaList[0].getValue());
		for (int i = 1; i < metaList.length; i++) {
			sb.append(FDFS_RECORD_SEPERATOR);
			sb.append(metaList[i].getName()).append(FDFS_FIELD_SEPERATOR).append(metaList[i].getValue());
		}
	 ...
	 * @return token string
	 */
	public static String getToken(String remoteFilename, int ts, String secretKey) throws UnsupportedEncodingException
			, NoSuchAlgorithmException {
		byte[] bsFilename = remoteFilename.getBytes(ClientGlobal.G_CHARSET);
		byte[] bsKey = secretKey.getBytes(ClientGlobal.G_CHARSET);
	...
	public static String genSlaveFilename(String master_filename,
		...
		if (ext_name != null) {
			...
			if (dotIndex < 0) {
				...
				trueExtName = master_filename.substring(dotIndex);
			}
		}

		if (trueExtName.length() == 0 && "-m".equals(prefix_name)) {
			throw new FastDfsException("prefix_name \"" + prefix_name + "\" is invalid");
		}

		if (dotIndex < 0) {
			return master_filename + prefix_name + trueExtName;
		} else {
			return master_filename.substring(0, dotIndex) + prefix_name + trueExtName;
		}
	}
